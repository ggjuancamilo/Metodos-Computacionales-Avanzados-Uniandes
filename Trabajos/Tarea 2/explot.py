...
if os.path.isfile('data1'):
	...
	for i in range(np.size(x1)):
		if (np.isnan(x1[i])||(x1[i]==inf)):
			x1[i] = 1.0
		if (np.isnan(y1[i])||(y1[i]==inf)):
			y1[i] = 1.0
		if (np.isnan(z1[i])||(z1[i]==inf)):
			z1[i] = 1.0
		if (np.isnan(rho1[i])||(rho1[i]==inf)):
			rho1[i] = 1.0
...
if os.path.isfile('data2'):
	...
	rho2 = data2[:,3]
	for i in range(np.size(x1)):
		if (np.isnan(x2[i])||(x2[i]==inf)):
			x2[i] = 1.0
		if (np.isnan(y2[i])||(y2[i]==inf)):
			y2[i] = 1.0
		if (np.isnan(z2[i])||(z2[i]==inf)):
			z2[i] = 1.0
		if (np.isnan(rho2[i])||(rho2[i]==inf)):
			rho2[i] = 1.0
	rhon2 = np.array([])
...
if os.path.isfile('data3'):
	...
	rho3 = data3[:,3]
	for i in range(np.size(x1)):
		if (np.isnan(x3[i])||(x3[i]==inf)):
			x3[i] = 1.0
		if (np.isnan(y3[i])||(y3[i]==inf)):
			y3[i] = 1.0
		if (np.isnan(y3[i])||(y3[i]==inf)):
			y3[i] = 1.0
		if (np.isnan(z3[i])||(z3[i]==inf)):
			z3[i] = 1.0	
		if (np.isnan(rho3[i])||(rho3[i]==inf)):
			rho3[i] = 1.0
		rhon3 = np.array([])
